...
function getNextjsConfigMjsTemplate(withSentryConfigOptionsTemplate) {
...
function getNextjsConfigCjsAppendix(withSentryConfigOptionsTemplate) {
    return `
...
function getNextjsConfigEsmCopyPasteSnippet(withSentryConfigOptionsTemplate) {
    return `
...
function getClientIntegrationsSnippet(features) {
    if (features.replay) {
        return `
...
function getSimpleUnderscoreErrorCopyPasteSnippet() {
    return `
${chalk_1.default.green(`import * as Sentry from '@sentry/nextjs';`)}
${chalk_1.default.green(`import Error from "next/error";`)}

${chalk_1.default.dim('// Replace "YourCustomErrorComponent" with your custom error component!')}
YourCustomErrorComponent.getInitialProps = async (${chalk_1.default.green('contextData')}) => {
  ${chalk_1.default.green('await Sentry.captureUnderscoreErrorException(contextData);')}

  ${chalk_1.default.dim('// ...other getInitialProps code')}
...
exports.getSentryDefaultGlobalErrorPage = getSentryDefaultGlobalErrorPage;
function getGlobalErrorCopyPasteSnippet(isTs) {
    if (isTs) {
        return `"use client";

${chalk_1.default.green('import * as Sentry from "@sentry/nextjs";')}
${chalk_1.default.green('import Error from "next/error";')}
${chalk_1.default.green('import { useEffect } from "react";')}

export default function GlobalError(${chalk_1.default.green('{ error }: { error: Error }')}) {
  ${chalk_1.default.green(`useEffect(() => {
    Sentry.captureException(error);
  }, [error]);`)}
...
export default function GlobalError(${chalk_1.default.green('{ error }')}) {
  ${chalk_1.default.green(`useEffect(() => {
    Sentry.captureException(error);
  }, [error]);`)}
